...
PopupBase {
    ...
    function _crossfadeText() {
        const meta = root.getCleanMetadata(root.activePlayer?.trackTitle, root.activePlayer?.trackArtist);

        let id = root.activePlayer?.identity ?? "";
        const idLow = id.toLowerCase();
        if (idLow.includes("firefox"))
            id = "Firefox";
        else if (idLow.includes("zen"))
            id = "Zen";
        else if (idLow.includes("chrome"))
            id = "Chrome";
        else if (idLow.includes("spotify"))
            id = "Spotify";
        ...
        if (cur.title === newTitle && cur.artist === newArtist && cur.identity === id)
            return;
            ...
            Text {
                ...
                SequentialAnimation {
                    ...
                    NumberAnimation {
                        ...
                        property: "x"
                        from: 0
                        to: -slotTitle.implicitWidth
                        duration: slotTitle.implicitWidth * 14
                        easing.type: Easing.Linear
                    }
                    PropertyAction {
                        target: slotTitle
                        property: "x"
                        value: slotTitle.parent.width
                    }
                    NumberAnimation {
                        target: slotTitle
                        property: "x"
                        from: slotTitle.parent.width
                        to: 0
                        duration: slotTitle.implicitWidth * 10
                        easing.type: Easing.Linear
                    }
                    PauseAnimation {
                        duration: 1200
    ...
    component WaveBar: Item {
        ...
        Canvas {
            ...
            Connections {
                ...
                function on_WaveAmountChanged() {
                    waveCanvas.requestPaint();
                }
                function onHoveredChanged() {
                    waveCanvas.requestPaint();
                }
                function on_StrokeColorChanged() {
                    waveCanvas.requestPaint();
                }
            }
        }
    ...
    component MediaButton: Rectangle {
        ...
        color: {
            if (!enabled)
                return Qt.rgba(PanelColors.rowBackground.r, PanelColors.rowBackground.g, PanelColors.rowBackground.b, 0.35);
            if (highlighted)
                return btnMouse.containsMouse ? Qt.lighter(accentColor, 1.1) : accentColor;
        ...
        Text {
            ...
            color: {
                if (!btn.enabled)
                    return PanelColors.textDim;
                if (btn.highlighted)
                    return PanelColors.pillForeground;
