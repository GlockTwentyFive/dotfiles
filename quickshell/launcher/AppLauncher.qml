...
PanelWindow {
    ...
    readonly property bool appModeActive: !wallpaperMode && !clipboardMode && !emojiMode && !hiddenMode
    ...
    readonly property int totalPages: isGridView ? Math.max(1, Math.ceil(filteredApps.length / 15)) : 1
    ...
    function _updateFilter() {
        ...
        for (var i = 0; i < appView.appsRepeaterCount; i++) {
            ...
            var nameMatch = q === "" || item.appName.toLowerCase().includes(q) || (item.appData && item.appData.genericName && String(item.appData.genericName).toLowerCase().includes(q)) || (item.appData && item.appData.comment && String(item.appData.comment).toLowerCase().includes(q)) || (item.appData && item.appData.keywords && String(item.appData.keywords).toLowerCase().includes(q));
    ...
    Rectangle {
        ...
        Column {
            ...
            LauncherSearchBar {
                ...
                rightPillText: {
                    ...
                    if (root.wallpaperMode) {
                        if (root.wallpaperMediaFilter === "all")
                            return " ALL";
                        if (root.wallpaperMediaFilter === "image")
                            return " IMG";
                        return " VID";
                    }
                    if (root.appModeActive)
                ...
                onRightPillClicked: {
                    if (root.clipboardMode) {
                        ...
                        if (root.wallpaperMediaFilter === "all")
                            root.wallpaperMediaFilter = "image";
                        else if (root.wallpaperMediaFilter === "image")
                            root.wallpaperMediaFilter = "video";
                        else
                            root.wallpaperMediaFilter = "all";
                    } else {
                        root.animateWidth = true;
                        root.isGridView = !root.isGridView;
                        filterTimer.restart();
                ...
                onTextChanged: {
                    ...
                    if (root.wallpaperMode)
                        wallpaperView.setFilter(t);
                    else if (root.clipboardMode)
                        clipboardView.setFilter(t);
                    else if (root.emojiMode)
                        emojiView.setFilter(t);
                ...
                onUpPressed: {
                    if (root.wallpaperMode)
                        wallpaperView.navigateUp();
                    else if (root.clipboardMode)
                        clipboardView.navigateUp();
                    else if (root.emojiMode)
                        emojiView.navigateUp();
                    else if (root.hiddenMode)
                        hiddenAppsView.navigateUp();
                    else
                        appView.navigateGrid(0, -1);
                }
                onDownPressed: {
                    if (root.wallpaperMode)
                        wallpaperView.navigateDown();
                    else if (root.clipboardMode)
                        clipboardView.navigateDown();
                    else if (root.emojiMode)
                        emojiView.navigateDown();
                    else if (root.hiddenMode)
                        hiddenAppsView.navigateDown();
                    else
                        appView.navigateGrid(0, +1);
                }
                onLeftPressed: {
                    if (root.wallpaperMode)
                        wallpaperView.navigateLeft();
                    else if (root.emojiMode)
                        emojiView.navigateLeft();
                    else if (root.hiddenMode && root.isGridView)
                        hiddenAppsView.navigateLeft();
                    else if (root.isGridView)
                        appView.navigateGrid(-1, 0);
                }
                onRightPressed: {
                    if (root.wallpaperMode)
                        wallpaperView.navigateRight();
                    else if (root.emojiMode)
                        emojiView.navigateRight();
                    else if (root.hiddenMode && root.isGridView)
                        hiddenAppsView.navigateRight();
                    else if (root.isGridView)
                        appView.navigateGrid(+1, 0);
                }
                onTabPressed: {
                    if (root.wallpaperMode)
                        wallpaperView.navigateTab();
                    else if (root.clipboardMode)
                        clipboardView.navigateTab();
                    else if (root.emojiMode)
                        emojiView.navigateTab();
                    else if (root.hiddenMode)
                        hiddenAppsView.navigateTab();
                    else
                        appView.navigateGrid(+1, 0);
                }
                onBacktabPressed: {
                    if (root.wallpaperMode)
                        wallpaperView.navigateBacktab();
                    else if (root.clipboardMode)
                        clipboardView.navigateBacktab();
                    else if (root.emojiMode)
                        emojiView.navigateBacktab();
                    else if (root.hiddenMode)
                        hiddenAppsView.navigateBacktab();
                    else
                        appView.navigateGrid(-1, 0);
                }
                onDeletePressed: {
                    if (root.clipboardMode)
                        clipboardView.deleteSelected();
                }
                onReturnPressed: {
                    if (root.wallpaperMode)
                        wallpaperView.confirm();
                    else if (root.clipboardMode)
                        clipboardView.confirm();
                    else if (root.emojiMode)
                        emojiView.confirm();
                    else if (root.hiddenMode)
                        hiddenAppsView.confirm();
                    else {
                    ...
                    GridView {
                        ...
                        delegate: Item {
                            ...
                            Rectangle {
                                ...
                                radius: 12
                                color: gridHiddenHover.containsMouse || hiddenAppsView.selectedIndex === index ? Qt.rgba(1, 1, 1, 0.08) : "transparent"
                                Behavior on color {
                                    ColorAnimation {
                                        duration: 150
                                    }
                    ...
                    ListView {
                        ...
                        delegate: Item {
                            ...
                            Rectangle {
                                ...
                                radius: 6
                                color: hiddenRowHover.containsMouse || hiddenAppsView.selectedIndex === index ? PanelColors.rowBackground : "transparent"
                                Behavior on color {
                                    ColorAnimation {
                                        duration: 120
                                    }
            ...
            LauncherAppView {
                ...
                readonly property int activeHeight: root.isGridView ? 412 : (function () {
                ...
                onPageChangeRequested: delta => {
                    ...
                    if (next >= 0 && next < root.totalPages)
            ...
            Row {
                ...
                readonly property bool shouldShow: root.appModeActive && root.isGridView && root.totalPages > 1
