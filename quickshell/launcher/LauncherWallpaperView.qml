...
Item {
    ...
    function _mediaType(path) {
        ...
        if (ext === "gif")
            return "gif";
        if (["mp4", "mkv", "webm", "mov", "avi", "flv", "wmv", "ts", "m4v", "ogv"].indexOf(ext) !== -1)
            return "video";
        return "image";
    }
    ...
    function _applyFilter() {
        ...
        for (var i = 0; i < wallpaperModel.count; i++) {
            ...
            var typeMatch = root.mediaFilter === "all" || (root.mediaFilter === "image" && e.mediaType !== "video") || (root.mediaFilter === "video" && e.mediaType === "video");
            if (!typeMatch)
                continue;
    ...
    Process {
        ...
        function generateAll(paths) {
            ...
            for (var i = 0; i < paths.length; i++) {
                ...
                cmds.push("[ -f '" + thumb + "' ] || " + "ffmpeg -y -ss 00:00:01 -i '" + p + "' " + "-vframes 1 -vf 'scale=256:-1' -q:v 3 '" + thumb + "' " + ">/dev/null 2>&1");
            }
    ...
    Process {
        ...
        function apply(path, mediaType) {
            ...
            if (mediaType === "video") {
                script = "pkill -x awww-daemon 2>/dev/null; " + "pkill -x mpvpaper 2>/dev/null; " + "while pgrep -x 'mpvpaper|awww-daemon' > /dev/null; do sleep 0.05; done; " + "mkdir -p \"$HOME/.cache/meloworld\"; " + "echo 'video:" + p + "' > \"$HOME/.cache/meloworld/last-wallpaper\"; " +
                // Extract frame at 1s → lockscreen wallpaper (runs in background, non-blocking)
                "ffmpeg -y -ss 00:00:01 -i '" + p + "' -vframes 1 " + lock + " >/dev/null 2>&1 & " + "mpvpaper -f -p -o '--loop-file=inf --no-audio --hwdec=auto' ALL '" + p + "'";
            } else {
                script = "pkill -x mpvpaper 2>/dev/null; " + "awww query >/dev/null 2>&1 || { awww-daemon &>/dev/null & " + "for i in $(seq 1 20); do sleep 0.1 && awww query >/dev/null 2>&1 && break; done; }; " + "mkdir -p \"$HOME/.cache/meloworld\"; " + "echo 'image:" + p + "' > \"$HOME/.cache/meloworld/last-wallpaper\"; " + "awww img '" + p + "' --transition-type fade --transition-duration 0.8 --transition-fps 60; " +
                ...
                Column {
                    ...
                    Item {
                        ...
                        Image {
                            ...
                            anchors.fill: parent
                            anchors.margins: 2
                            source: modelData.mediaType === "video" ? ("file://" + modelData.thumbPath) : ("file://" + modelData.filePath)
